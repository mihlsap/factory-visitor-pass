...
public class UserClearanceServiceImpl implements UserClearanceService {
     ...
     * <ul>
     * <li><b>Sequential Progression:</b> The logic checks levels 1 through 4
     * sequentially.
     * If a user satisfies Level 1 but fails requirements for Level 2, the loop
     * breaks immediately.
     * This implies a user cannot hold Level 3 access without also satisfying Level
     * 2.</li>
     ...
     * <li>It has not been manually revoked
     * ({@code isTrainingRevoked == false}).</li>
     * </ol>
     * </li>
     * <li><b>Empty Levels:</b> If a specific security level has no assigned
     * trainings in the system,
     * the user <b>cannot</b> achieve this level (progression stops).</li>
     * <li><b>Optimisation:</b> Database update and audit log occur only if the
     * calculated level
    ...
    public void recalculateUserClearance(UUID userId) {
        ...
        for (int level = 1; level <= 4; level++) {
            ...
            // Edge case: No trainings defined for this level -> Stop progression
            ...
            for (Training reqTraining : requiredTrainings) {
                boolean completed = userStatuses.stream()
                        .anyMatch(status -> status.getTraining().getId().equals(reqTraining.getId()) &&
                                status.getStatus() == ProgressStatus.COMPLETED &&
                                (status.getValidUntil() == null
        ...
        if (user.getClearanceLevel() != newClearance) {
            ...
            auditLogService.logEvent(user.getId(), AppMessage.CLEARANCE_CHANGED.name(),
