...
public class VerificationServiceImpl implements VerificationService {
     ...
     * <ul>
     * <li><b>Just-In-Time Recalculation:</b> Before making a decision, this method
     * calls
     * {@link UserClearanceService#recalculateUserClearance(UUID)}. This forces a
     * fresh check
     * of all expiration dates. Even if the background scheduler hasn't run yet, an
     * expired
     * training will be detected <i>at the moment of scan</i>, preventing
     ...
     * <li><b>Visual Verification:</b> Returns the user's name and photo URL even if
     * access is denied
     * (unless the user is not found), allowing the guard to verify if the person
     * holding the pass is the owner.</li>
     * </ul>
     * </p>
     */
    @Override
    ...
    public VerificationResponse verifyUserAccess(UUID userId, int requiredLevel) {
        ...
        if (user.getStatus() != UserStatus.ACTIVE) {
            auditLogService.logEvent(user.getId(), AppMessage.ACCESS_DENIED_BLOCKED.name(),
        ...
        if (user.getClearanceLevel() >= requiredLevel) {
            auditLogService.logEvent(user.getId(), AppMessage.ACCESS_GRANTED.name(),
            ...
            auditLogService.logEvent(user.getId(), AppMessage.ACCESS_DENIED_LOW_LEVEL.name(),
