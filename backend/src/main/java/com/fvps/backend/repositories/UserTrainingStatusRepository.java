 ...
 * <p>
 * This repository handles the intersection between Users and Trainings. It is
 * heavily used
 * for tracking course progress, retrieving assigned trainings for dashboard
 * views,
 * and monitoring pass validity expiration.
 * </p>
 */
@Repository
public interface UserTrainingStatusRepository
        extends JpaRepository<UserTrainingStatus, UUID>, JpaSpecificationExecutor<UserTrainingStatus> {

        /**
         * Retrieves a paginated list of training statuses for a specific user.
         * <p>
         * <b>Performance Note:</b> Uses {@code @EntityGraph} to eagerly fetch the
         * associated
         ...
         * <p>
         * Similar to the paginated version, this method eagerly loads associated
         * entities.
         ...
         * <p>
         * Used for reporting (e.g. "Show me all users currently assigned to Fire
         * Safety").
         * </p>
         *
         ...
         * <p>
         * Typically called when a Training definition is deleted from the system,
         * ensuring
         ...
         * <p>
         * Used when a user enters a training to determine where they left off (current
         * module)
        ...
        /**
         * Finds all records that are expired (validUntil < now) but still marked as
         * generic status (e.g. COMPLETED).
         *
         * @param status   current status (e.g. COMPLETED).
         * @param dateTime the cutoff time (now).
         * @return list of expired records.
         */
        @EntityGraph(attributePaths = { "user", "training" })
        List<UserTrainingStatus> findAllByStatusAndValidUntilBefore(ProgressStatus status, LocalDateTime dateTime);

        /**
         * Finds records based on status and validity expiration date range.
         * <p>
         * <b>Use Case:</b> This method is primarily used by background scheduler jobs
         * (Cron) to find users whose certificates are about to expire (or have just
         * expired) in order
         * to send automated email notifications.
         * </p>
         * <p>
         * Eagerly fetches {@code user} and {@code training} to facilitate email
         * generation without
         * extra queries.
         * </p>
         *
        ...
        /**
         * Retrieves a list of distinct user IDs who have been assigned any of the
         * specified trainings.
         * <p>
         * This query checks for the existence of a {@link UserTrainingStatus} record
         * associated with any
         * of the provided training IDs, regardless of the user's progress status (e.g.,
         * IN_PROGRESS, COMPLETED, FAILED).
         * It effectively identifies users who are participants in the given set of
         * trainings.
         * </p>
         *
         * @param trainingIds the list of training UUIDs to check for assignments.
         * @return a list of distinct user UUIDs who have at least one of the specified
         *         trainings assigned.
         */
        @Query("SELECT DISTINCT s.user.id FROM UserTrainingStatus s WHERE s.training.id IN :trainingIds")
        List<UUID> findUserIdsByAssignedTrainingIds(@Param("trainingIds") List<UUID> trainingIds);
}