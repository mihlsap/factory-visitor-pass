...
public class ResetPasswordRequest {
        ...
        @Schema(description = "New password (must meet security requirements)", example = "NewStrongPass2@", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "New password is required")
        @Pattern(regexp = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=!])(?=\\S+$).{8,}$", message = "Password must be at least 8 characters long, contain uppercase and lowercase letter, digit and special character.")
